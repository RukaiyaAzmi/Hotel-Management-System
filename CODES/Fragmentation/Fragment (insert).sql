...
begin
	...
	loop
		...
		IF cus_city = 'Sylhet'
		THEN insert into Customer1@site_link1 values(cus_id,cus_name,cus_mobile,cus_email,cus_date_of_birth,cus_nationality,cus_city);
	...
	loop
		...
		IF a_name = 'Prima'
		THEN insert into Admin1@site_link1 values(a_id,a_name,a_address,a_email,a_mobileno);
	...
	loop
		...
		IF r_rate > 12000 
		THEN insert into RoomDetails1@site_link1 values(r_no,r_type,r_rate,r_review);
	...
	loop
		...
		IF re_cid = 4
		THEN insert into Reservation1@site_link1 values(re_no,re_cid,re_roomno,re_checkin,re_checkout);
	...
	loop
		...
		IF e_name = 'Rahim'  
		THEN insert into Employee1@site_link1 values(e_id,e_name,e_duty,e_roomno);
	...
	loop
		...
		IF f_rate >= 400 
		THEN insert into Food1@site_link1 values(f_no,f_type,f_name,f_rate);
	...
	loop
		...
		IF b_amount >=25000
		THEN insert into Bill1@site_link1 values(b_id,b_cid,b_roomno,b_amount,b_pstatus);
